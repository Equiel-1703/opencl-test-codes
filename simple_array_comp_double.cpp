...
int main(int argc, char const *argv[])
{
  ...
  const char *kernel_code = R"(
    ...
    __kernel void square_array(__global const double* input, __global double* output) {
  ...
  double *output_array = new double[array_size];

  // Initialize input array
  for (size_t i = 0; i < array_size; ++i)
  {
    input_array[i] = static_cast<double>(i);
  }

  // Create buffers (device memory)
  cl::Buffer input_buffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, array_size * sizeof(double), input_array);
  cl::Buffer output_buffer(context, CL_MEM_WRITE_ONLY, array_size * sizeof(double));
  ...
  queue.enqueueReadBuffer(output_buffer, CL_TRUE, 0, array_size * sizeof(double), output_array);
